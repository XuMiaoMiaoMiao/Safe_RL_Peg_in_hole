#!/bin/bash
# FINAL Stage 1 benchmark — SAC vs LagSAC, harder pose, 15 seeds, 60ep.
# 2026-05-24 rerun: same params as the 2026-05-20 verified set, extended to
# 15 seeds (0..14) and benefiting from the rollout cost tracker refactor
# (real rollout_ep_max_violation / rollout_ep_cost in logs; strict-mode plot).
#
...
#   SAC 60ep ≈ 17min × 15 = 255 min
#   + IsaacSim init overhead 30 × 30s = 15 min
#   = ~9.75 h  (tight; one PhysX hang adds ~30 min)
#
...
SAC vs LagSAC, harder pose, 15 paired seeds × 60ep.
...
Eval framework: best_hold.msh checkpoint, mean ± std over completed seeds.
Expected: ~9.75h total. timeout=${RUN_TO}/run.
DRY_RUN=$DRY_RUN
...
echo ""
echo "============================================================"
echo "Done. Benchmark summary: cat /tmp/final_bench_summary.txt"
