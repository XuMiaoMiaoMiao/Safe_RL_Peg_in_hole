...
def compute_geom_metrics(dataset, mdp, hold_n_steps):
    """Geometric metrics for geom_stage={prepos,preaxis,insert}.

    This is the single source used by SAC eval/training and Lagrangian SAC.
    The active success mask comes from env info and follows mdp._geom_stage.
    For insert, entry diagnostics are measured at the first step with d > 0
    (the peg has crossed the hole entry plane), so entry penetration can expose
    scraping before the stricter insert success mask is satisfied. For prepos
    and preaxis, entry remains the first active success-mask step.
    """
    _, _, _, _, _, last = dataset.parse(to="torch")
    info = dataset.info.data
    required = (
        "geom_d", "geom_d_target", "geom_radial_tip", "geom_radial_max",
        "geom_axis_err", "geom_penetration_max", "geom_success_mask",
        "geom_prepos_mask", "geom_preaxis_mask", "geom_insert_mask",
    )
    ...
    d = _to_tensor(info["geom_d"]).float()
    d_target = _to_tensor(info["geom_d_target"]).float()
    d_err = torch.abs(d - d_target)
    radial_tip = _to_tensor(info["geom_radial_tip"]).float()
    radial_max = _to_tensor(info["geom_radial_max"]).float()
    axis_err = _to_tensor(info["geom_axis_err"]).float()
    penetration = _to_tensor(info["geom_penetration_max"]).float()
    success = _to_tensor(info["geom_success_mask"]).float() > 0.5
    prepos = _to_tensor(info["geom_prepos_mask"]).float()
    preaxis = _to_tensor(info["geom_preaxis_mask"]).float()
    insert = _to_tensor(info["geom_insert_mask"]).float()
    ...
    entry_indices = []
    entry_mode_insert = getattr(mdp, "_geom_stage", None) == "insert"
    start = 0
    for end in end_indices:
        ...
        if entry_mode_insert:
            entry = np.flatnonzero(d_np[start:end + 1] > 0.0)
        else:
            entry = np.flatnonzero(ep_success)
        if len(entry):
    ...
    active_pen = penetration[success]
    clean_threshold = float(getattr(mdp, "_geom_pen_th", 1e-4))
    ...
    return {
        ...
        "geom_prepos_step_rate": float(prepos.mean()),
        "geom_preaxis_step_rate": float(preaxis.mean()),
        "geom_insert_step_rate": float(insert.mean()),
        ...
        "geom_clean_step_rate": float((penetration < clean_threshold).float().mean()),
        "geom_pen_in_active_mean": (
            float(active_pen.mean()) if active_pen.numel() else float("nan")
        ),
        "geom_pen_in_active_max": (
            float(active_pen.max()) if active_pen.numel() else float("nan")
        ),
        "geom_n_ep_with_entry": len(entry_indices),
        "geom_entry_d_mean": _mean_at(d, entry_idx),
        "geom_entry_d_err_mean": _mean_at(d_err, entry_idx),
        "geom_entry_radial_max_mean": _mean_at(radial_max, entry_idx),
        "geom_entry_radial_max_max": _max_at(radial_max, entry_idx),
        "geom_entry_axis_err_mean": _mean_at(axis_err, entry_idx),
        "geom_entry_axis_err_max": _max_at(axis_err, entry_idx),
        "geom_entry_penetration_mean": _mean_at(penetration, entry_idx),
        "geom_entry_penetration_max": _max_at(penetration, entry_idx),
        "geom_final_d_mean": _mean_at(d, final_idx),
        "geom_final_d_err_mean": _mean_at(d_err, final_idx),
        "geom_final_radial_max_mean": _mean_at(radial_max, final_idx),
        "geom_final_radial_max_max": _max_at(radial_max, final_idx),
        "geom_final_axis_err_mean": _mean_at(axis_err, final_idx),
        "geom_final_axis_err_max": _max_at(axis_err, final_idx),
        "geom_final_penetration_mean": _mean_at(penetration, final_idx),
        "geom_final_penetration_max": _max_at(penetration, final_idx),
    }
