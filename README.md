...
`mean ± 1.96·σ/√n`). Also available via `--band`: `sem` (`±σ/√n`),
`std` (`±σ`), or `none`.
...
[`scripts/plot_paper_benchmark.py`](scripts/plot_paper_benchmark.py). This is a
legacy replot of the 2026-05-20 logs; the max-violation panel uses the proxy
noted below because those logs predate the rollout max tracker.

**Headline (epoch 60, mean [95% CI])**

| Metric | LagSAC | SAC | LagSAC vs SAC |
|---|---:|---:|---|
| Discounted Return (J) | −89.0 [−113.6, −64.4] | −99.7 [−128.4, −70.9] | tie / mild Lag |
| Maximum Violation per Episode¹ | 0.067 [0.031, 0.102] | 0.076 [0.021, 0.131] | tie (legacy proxy) |
| Episodic Sum of Cost | **4.34** [0.76, 7.92] | 6.62 [2.64, 10.59] | **≈ 1.52× safer** |
